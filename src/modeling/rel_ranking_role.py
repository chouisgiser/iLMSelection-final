...
def ranking():
    for fold_index in range(1, 6):
        for familiarity in ['familiar', 'unfamiliar']:
            ...
            results_dir = '../../results/'
            hyperparams_dir = '../../results/experiments/hyperparams'
...
def ranking_iteration(split_time):
    for fold_index in range(1, 6):
        for role in ['staff', 'student']:
            data_dir = '../../data/processed/dataset_split_iteration/iter' + str(split_time) + '/ranking/5folds_role/fold'
            results_dir = '../../results/dataset_split_iteration/role/iter' + str(split_time) + '/'
            hyperparams_dir = '../../results/dataset_split_iteration/role/iter' + str(split_time) + '/experiments/hyperparams/'
            ...
            df_hyperparams = pd.read_csv(hyperparams_dir + '/lgbrank_' + role + '_params.csv')
            hyperparams = df_hyperparams[(df_hyperparams['fold'] == fold_index)].iloc[-1, :]
            ...
            print('Print importances:', list(ranker.feature_importances_))
            file_name = results_dir + 'experiments/feature_importance/ranking_' + role + '/fold' + str(
                fold_index) + '_importance.csv'
            column_name = ['features', 'importance_gain']
            column_data = {'features': ranker.feature_name_, 'importance_gain': list(ranker.feature_importances_)}
            pd_eval_his = pd.DataFrame(columns=column_name, data=column_data)
            ...
            print('Saving model...')
            ranker.booster_.save_model(
                results_dir + 'models/ranking_' + role + '/fold' + str(fold_index) + '_model.txt')
# ranking()
